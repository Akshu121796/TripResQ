...
function getInstantCoordsForQuery(
  query = '',
  fallbackName = 'Mumbai Airport'
) {
...
function deriveNextStopFromNodes(nodes, destinationFallback) {
  ...
  const hotelNode = nodes.find(
    n => (n.type || '').toLowerCase() === 'hotel'
  );
  ...
  if (hotelNode) {
    return {
      ...
      destination: dest,
      arrival_time:
        hotelNode.scheduledStart ||
        hotelNode.actualStart ||
        '13:00',
      latitude: null,
      longitude: null
    };
  ...
  const trainOrFlight = nodes.find(
    n =>
      (n.type || '').toLowerCase() === 'train' ||
      (n.type || '').toLowerCase() === 'flight'
  );
  ...
  if (trainOrFlight) {
    ...
    return {
      ...
      destination: dest,
      arrival_time:
        trainOrFlight.scheduledEnd ||
        trainOrFlight.actualEnd ||
        '19:35',
      latitude: null,
      longitude: null
    };
 ...
 *
 * Features:
 * - Instant bundled restaurant fallback for demo reliability
 * - Centralized backend API through apiFetch / DB_API_URL
 * - Next-stop based dining
 * - Current location dining
 * - Manual location search
 * - OpenStreetMap Overpass live search
...
export default function DiningHub({
  ...
  const initialStop = deriveNextStopFromNodes(
    currentTripNodes,
    activeDestination
  );

  const initialLocQuery =
    initialStop?.destination ||
    initialStop?.name ||
    activeDestination ||
    'Mumbai Airport';

  const initialCoords = getInstantCoordsForQuery(
    initialLocQuery,
    initialStop?.name || activeDestination
  );
  ...
  // IMPORTANT:
  // Load restaurants immediately from the bundled fallback dataset.
  // This means demo cards are visible even if backend / Overpass is down.
  ...
  const effectiveTripId =
    tripId || tripRef || 'TR-998827';


  // ------------------------------------------------------------
  // Keep demo fallback data synchronized with destination
  // ------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    const fallbackRestaurants = getFallbackRestaurants(city);

    setRawRestaurants(fallbackRestaurants);

    setResolvedLocationName(`${city} Railway Station`);

    setNextStopData(
      deriveNextStopFromNodes(currentTripNodes, city)
    );
  ...
  // ------------------------------------------------------------
  // Fetch next stop through centralized API
  // ------------------------------------------------------------
  ...
  const fetchNextStop = useCallback(async () => {
    ...
    try {
      ...
      const resp = await apiFetch(
        `/trips/${effectiveTripId}/next-stop`,
        {
      ...
      // Use local demo fallback instead.
      const fallbackStop = deriveNextStopFromNodes(
        currentTripNodes,
        activeDestination
      );
      ...
      console.warn(
        '[dining] Next-stop backend unavailable, using demo fallback:',
        err
      );
  ...
  // ------------------------------------------------------------
  // Resolve coordinates based on selected mode
  // ------------------------------------------------------------

  const resolveLocationForMode = useCallback(
    async (
      mode,
      customQuery = null,
      stopOverride = null
      ...
      try {
        ...
        if (mode === 'next_stop') {
          ...
          const geocoded = await searchLocation(
            queryToGeocode
          );
          ...
          if (geocoded) {
            ...
            // Immediately show bundled restaurant cards.
            setRawRestaurants(
              getCuratedFallbackRestaurants(
                geocoded.lat,
                geocoded.lng,
            ...
            // Even if geocoding fails, preserve demo data.
            const instant = getInstantCoordsForQuery(
              queryToGeocode,
              stop?.name || activeDestination
            );

            setCurrentCoords(instant);
            ...
            setLocationError(
              "Live location lookup failed, so TripResQ is showing the bundled dining results."
            );
          }
        }
        ...
        // ------------------------------------------------------
          ...
          try {
            ...
            console.log(
              `[dining] mode=CURRENT_LOCATION, location=${userLoc.name}`
            );
          } catch (geoErr) {
            console.warn(
              '[dining] Geolocation failed:',
              geoErr.message
            );

            setLocationError(
              'Location access unavailable. Search an area instead.'
            );
          } finally {
        ...
        // ------------------------------------------------------
          ...
          try {
            ...
            if (geocoded) {
              ...
              // Always populate something immediately.
              setRawRestaurants(
                getCuratedFallbackRestaurants(
                  geocoded.lat,
                  geocoded.lng,
              ...
              console.log(
                `[dining] mode=SEARCH, location=${locName}`
              );
            } else {
              setLocationError(
                `Could not find coordinates for "${q}". Please try a nearby station, landmark, or city.`
              );
            }
          } finally {
            setIsLoadingLocation(false);
        ...
        console.error(
          '[dining] Location resolution error:',
          err
        );

        setLocationError(
          'Error determining location coordinates.'
        );
      }
    },
    [
  ...
  // It does NOT block the bundled restaurant cards.
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // Live OpenStreetMap / Overpass restaurant query
  //
  // IMPORTANT:
  // We NEVER clear rawRestaurants when Overpass fails.
  // Existing fallback cards stay visible.
  // ------------------------------------------------------------
  ...
  useEffect(() => {
    ...
    getNearbyRestaurants(
      ...
      .catch((err) => {
        ...
        // Do NOT empty the restaurant state.
        // The bundled fallback remains visible.
        console.warn(
          '[dining] Overpass unavailable; keeping curated restaurant dataset:',
          err
        );
      });
  ...
  const hasVegOption =
    rawRestaurants.some(
      restaurant => restaurant.vegetarian === true
    );

  const hasOpenNowOption =
    rawRestaurants.some(
      restaurant => restaurant.openStatus?.isOpen === true
    );
    ...
    <div className="w-full space-y-6 text-left">
      ...
      <div className="border-b border-slate-205 pb-5 text-left">
        ...
        <p className="text-slate-500 text-xs mt-1">
          Real nearby eateries detected around your upcoming
          itinerary arrival hub or chosen location.
        </p>
        ...
        {selectedRestaurant && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
            ...
            >
              ...
              {(() => {
                const imgMeta =
                  selectedRestaurant.resolvedImage ||
                  getRestaurantImage(
                    selectedRestaurant
                  );
              ...
              <div className="space-y-3 mb-6">
                  ...
                  {selectedRestaurant.phone && (
                    <div className="flex justify-between">
                      ...
                      <a
                        ...
                        className="font-bold text-[#287DFA] hover:underline"
                      >
                        {selectedRestaurant.phone}
                      </a>
                ...
                {(
                  ...
                  selectedRestaurant.wheelchair ||
                  selectedRestaurant.internetAccess
                ) && (
